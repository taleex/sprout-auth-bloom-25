...
const ExpectedPayments: React.FC<ExpectedPaymentsProps> = ({ bills }) => {
    ...
    return (
      <div className="bg-card border border-finapp-border rounded-xl shadow-sm">
        <div className="p-6 border-b border-finapp-border/50">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-foreground text-lg">Expected Payments</h3>
                <p className="text-sm text-muted-foreground">Expense projections</p>
              </div>
            </div>
        ...
        <div className="p-12 text-center">
          ...
          <h4 className="font-semibold text-foreground mb-2">No expenses configured</h4>
          <p className="text-sm text-muted-foreground">Add rent, utilities, subscriptions, or other expenses to see projections</p>
        </div>
      </div>
    );
  }
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="border-t border-finapp-border/50 pt-4">
            <div className="flex justify-between items-center mb-3">
              <span className="font-semibold text-red-800">Monthly Totals</span>
            ...
            <div className="grid grid-cols-3 gap-2">
              {MONTH_NAMES.map((month, monthIndex) => (
                <div key={monthIndex} className="text-center p-2 rounded bg-red-50">
                  <div className="text-xs text-red-600 mb-1">{month}</div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-finapp-muted/30">
                    ...
                    <th className="text-center py-4 px-4 font-semibold text-foreground border-b border-finapp-border/30 bg-red-50/50 min-w-24">
                      Total
                    </th>
                  </tr>
                </thead>
                  ...
                  {bills.map((bill, index) => (
                    <tr key={bill.id} className={`hover:bg-finapp-muted/20 transition-colors ${index !== bills.length - 1 ? 'border-b border-finapp-border/20' : ''}`}>
                      ...
                      {MONTH_NAMES.map((_, monthIndex) => {
                        const amount = getBillAmountForMonth(bill, monthIndex);
                        return (
                          <td key={monthIndex} className="text-center py-4 px-3 text-red-600 font-medium">
                            {amount > 0 ? formatCurrency(amount) : '—'}
                          </td>
                        );
                      })}
                      <td className="text-center py-4 px-4 bg-red-50/50 font-semibold text-red-700">
                        {formatCurrency(getBillYearlyTotal(bill))}
                      </td>
                    </tr>
                  ))}
                  <tr className="border-t-2 border-red-200 bg-red-50/30">
                    <td className="py-4 px-4 font-semibold text-red-800 sticky left-0 bg-red-50/30 z-10 border-r border-finapp-border/20">
                      Monthly Total
                    </td>
                    {MONTH_NAMES.map((_, monthIndex) => (
                      <td key={monthIndex} className="text-center py-4 px-3 font-semibold text-red-700">
                        {formatCurrency(getMonthlyTotal(monthIndex))}
                      </td>
                    ))}
                    <td className="text-center py-4 px-4 bg-red-100/70 font-bold text-red-800">
                      {formatCurrency(getGrandTotal())}
                    </td>
                  </tr>
                </tbody>
